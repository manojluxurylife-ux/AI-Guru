...
export const LearningPortal: React.FC<LearningPortalProps> = ({
  ...
  const [generatedLessonSuite, setGeneratedLessonSuite] = useState<{
    ...
    quiz: Array<{ question: string; options: string[]; correctIdx: number; explanation: string }>;
  ...
  const handleOneClickGenerate = () => {
    ...
    const prompt = `Develop a curriculum lesson on "${lessonTopic}" for ${lessonGrade} in ${lessonSubject}.
    Create a JSON format with this structure:
    {
      ...
      "quiz": [
        {"question": "Identify the primary law defined by this equation.", "options": ["Law A", "Law B", "Law C", "Law D"], "correctIdx": 1, "explanation": "This represents the fundamental physical principle of conservation."},
        {"question": "What happens if we double the temperature coefficient?", "options": ["Halves", "Doubles", "Stays same", "Quadruples"], "correctIdx": 1, "explanation": "Double temperature yields direct linear doubling of pressure in absolute terms."}
      ]
    }`;
                ...
                <div className="border border-slate-800/80 bg-slate-950/40 rounded-xl p-3.5 space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold uppercase font-mono ${voiceCloned ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-slate-900 text-slate-500'}`}>
                      {voiceCloned ? 'Cloned Active' : 'Offline'}
                    </span>
                  </div>
                  <p className="text-[9px] text-slate-500 leading-normal">
                    Upload a 5-second voice sample (.wav) to clone your own voice or target teachers for custom-cloned lectures.
                  </p>
                    ...
                    <label className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-slate-900 hover:bg-slate-850 text-slate-300 hover:text-white border border-slate-800 rounded-xl cursor-pointer transition text-[9px] font-bold uppercase">
                      ...
                      <input
                        ...
                        onChange={() => {
                          ...
                          setTimeout(() => {
                            setVoiceCloned(true);
                            setSelectedVoice('Custom Cloned Student Voice (VoxCPM2)');
                          }, 1000);
                        }}
                      />
                    </label>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl flex flex-col flex-1 min-h-[350px]">
              ...
              <div className="p-3.5 bg-slate-950/80 border-t border-slate-800 flex gap-2">
                <input
                  ...
                  className="flex-1 bg-slate-900 border border-slate-800 rounded-xl px-3.5 py-2 text-[11px] outline-none text-white placeholder-slate-500 focus:border-indigo-500"
                />
                <button
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col gap-6">
              ...
              <div className="space-y-4 mt-4">
                ...
                <div className="border-2 border-dashed border-slate-800 hover:border-indigo-500/50 bg-slate-950/40 rounded-xl p-4 transition-all text-center">
                  ...
                  <label className="mt-3 inline-block cursor-pointer bg-slate-900 hover:bg-slate-800 text-[9px] font-black text-slate-300 border border-slate-850 rounded-lg px-3 py-1.5 transition">
                    {pdfFileUploaded ? 'Document Uploaded' : 'Choose Document'}
                    <input
                      type="file"
                      accept=".pdf,.txt,.docx"
                      className="hidden"
                      onChange={() => setPdfFileUploaded(true)}
                    />
                  </label>
                </div>
            ...
            <div className="border border-slate-800/80 bg-slate-950/40 rounded-xl p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <button
                  onClick={() => alert('PPTX compilation starting! Editable PPTX schema compiled dynamically for offline slideshow imports.')}
                  className="px-3 py-2 bg-slate-900 hover:bg-slate-850 text-slate-300 hover:text-white border border-slate-800 rounded-xl transition text-[9px] font-bold uppercase flex items-center justify-center gap-1.5"
                >
              ...
              <button
                onClick={() => alert('Classroom successfully packed as offline .maic.zip file for container deployment!')}
                className="w-full py-2.5 bg-slate-950 hover:bg-slate-900 text-indigo-400 hover:text-white border border-indigo-900/40 rounded-xl transition text-[9px] font-black uppercase tracking-wider"
              >
                📦 Export as offline .maic.zip
              </button>
            </div>
          </div>
                    ...
                    <div>
                      {quizScorecard ? (
                        <p className="text-xs font-black uppercase text-amber-500">
                          Scorecard: {quizScorecard.score} / {generatedLessonSuite.quiz.length} Correct ({Math.round((quizScorecard.score / generatedLessonSuite.quiz.length) * 100)}%)
                        </p>
                      ) : (
                    ...
                    <button
                      onClick={() => {
                        ...
                        generatedLessonSuite.quiz.forEach((q, qIdx) => {
                          if (studentQuizAnswers[qIdx] === q.correctIdx) {
                            score++;
                          }
                        });
